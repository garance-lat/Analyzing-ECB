#!/usr/bin/env python3
# ingest_normalize_ecb.py
# Usage:
#   python ingest_normalize_ecb.py "all_ECB_speeches.csv" "ecb_conferences_1998_2025.csv" \
#   -o ecb_text_corpus.csv --qa ecb_text_corpus_QA.json
...
def parse_best_date(df: pd.DataFrame) -> Tuple[Optional[str], Optional[pd.Series]]:
    candidates = [c for c in df.columns if any(k in c.lower() for k in ["date","time","year"])]
    best, best_ok, best_series = None, -1.0, None
    for c in candidates:
        s = pd.to_datetime(df[c], errors="coerce", infer_datetime_format=True, utc=False)
...
def normalize_df(df: pd.DataFrame, source_name: str, fallback_channel: str) -> tuple[pd.DataFrame, Dict[str,str]]:
    # tes colonnes : date | speakers | title | subtitle | contents
    title_col   = pick_col(df, ["title","subject","headline"])
    # "contents" correspond à "content" -> détecté
    text_col    = pick_col(df, ["text","content","speech","remarks","transcript","body"])
    # "speakers" contient "speaker" -> détecté
    speaker_col = pick_col(df, ["speaker","author","name","president"])
...
def main():
    ...
    ap.add_argument("csv_a")  # ex: all_ECB_speeches.csv
    ap.add_argument("csv_b")  # ex: ecb_conferences_1998_2025.csv
    ap.add_argument("-o","--output", default="ecb_text_corpus.csv")
    ap.add_argument("--qa", default="ecb_text_corpus_QA.json")
    args = ap.parse_args()
